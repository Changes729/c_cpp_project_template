/**
 * setcap cap_sys_admin+ep ./build/bin/demo_fanotify2
 * setcap cap_dac_read_search+ep ./build/bin/demo_fanotify2
 */
#define _GNU_SOURCE
#include <errno.h>
#include <fcntl.h>
...
int main(int argc, char *argv[])
{
  ...
  while(1) {
        ...
        metadata = FAN_EVENT_NEXT(metadata, size))
    {
      ...
      if(metadata->mask == FAN_CREATE) printf("FAN_CREATE (file created):\n");

      if(metadata->mask == (FAN_CREATE | FAN_ONDIR))
        printf("FAN_CREATE | FAN_ONDIR (subdirectory created):\n");

      if(metadata->mask == FAN_DELETE) printf("FAN_DELETE (file deleted):\n");

      if(metadata->mask == (FAN_DELETE | FAN_ONDIR))
        printf("FAN_DELETE | FAN_ONDIR (subdirectory deleted):\n");

      if(metadata->mask == (FAN_MOVED_FROM))
        printf("FAN_MOVE_FROM (file moved from):\n");

      if(metadata->mask == (FAN_MOVED_TO))
        printf("FAN_MOVE_TO (file moved to):\n");
      ...
      if(file_name) {
        ...
        if(ret == -1) {
          ...
          printf("\tEntry '%s:%lu' does not exist.\n", file_name, sb.st_ino);
