...
/// in server-bindings.h has one method not declear.
/// so, we need include `glib-dbus-server.h` first to declear it.
#include "glib-dbus-server.h"
...
static void server_class_init(ServerClass *klass) {
  ...
  dbus_g_object_type_install_info(TYPE_SERVER,
...
static gint server_init(Server *server) {
  ...
  dbus_g_connection_register_g_object(
      klass->connection, "/org/gnome/ServiceName", G_OBJECT(server));

  /* Register the service name, the constant here are defined in
   * dbus-glib-bindings.h */
  driver_proxy = dbus_g_proxy_new_for_name(klass->connection, DBUS_SERVICE_DBUS,
                                           DBUS_PATH_DBUS, DBUS_INTERFACE_DBUS);

  if (!org_freedesktop_DBus_request_name(
          driver_proxy, "org.gnome.ServiceName", 0,
...
gboolean server_echo_string(Server *server, gchar *original, gchar **echo,
                            GError **error) {
  *echo = g_strdup(original);
  ...
  if (problem) {
    ...
    g_set_error(error, g_quark_from_static_string("echo"), 0xdeadbeef,
