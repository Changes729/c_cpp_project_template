...
static void client_echo_reply(DBusGProxy *proxy, char *answer, GError *error,
                              gpointer userdata);
...
static void client_class_init(ClientClass *klass) {
  ...
  klass->proxy = dbus_g_proxy_new_for_name(
      klass->connection, "org.gnome.ServiceName", "/org/gnome/ServiceName",
...
static void client_send_async(Client *client) {
   ...
   */
  org_gnome_ServiceName_echo_string_async(
      klass->proxy, "The async request string we want echo-ed",
...
static void client_send_sync(Client *client) {
  ...
  if (!org_gnome_ServiceName_echo_string(
          klass->proxy, "The sync request string we want echo-ed", &result,
          &error)) {
...
static void client_echo_reply(DBusGProxy *proxy, char *answer, GError *error,
  ...
  if (error != NULL) {
    g_warning("An error occured while calling echo_string remote method: %s",
              error->message);
    g_error_free(error);
    return;
  }
